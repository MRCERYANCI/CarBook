...
namespace CarBook.WebUI.Areas.CarBookAdmin.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        public async Task<IActionResult> CreateCategory(CreateCategoryDto createCategoryDto)
        {
            ...
            {
                return RedirectToAction("Index", "Category", new { area = "CarBookAdmin" });
            }
            return View();
        }
        ...
        public async Task<IActionResult> DeleteCategory(int CategoryId)
        {
            ...
            var responseMessage = await client.DeleteAsync($"https://localhost:7125/api/Category?CategoryId={CategoryId}");
            if (responseMessage.IsSuccessStatusCode)//Eğer istek attığımız apiden(responsemessage) 200-299 arası durum kodu dönerse
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responsemessage = await client.GetAsync($"https://localhost:7125/api/Category/{CategoryId}");
            if (responsemessage.IsSuccessStatusCode)//200 ile 299 arasında bir sayı dönerse true döneceğinden başarılı false dönerse başarısız
            {
